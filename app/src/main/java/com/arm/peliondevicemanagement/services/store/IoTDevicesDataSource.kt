...
import com.arm.peliondevicemanagement.services.data.IoTDevicesResponse
...
class IoTDevicesDataSource(
    ...
    companion object {
        private val TAG: String = IoTDevicesDataSource::class.java.simpleName
    }

    override fun loadInitial(
        params: LoadInitialParams<String>,
        callback: LoadInitialCallback<String, IoTDevice>
    ) {
        scope.launch {
    ...
    override fun loadAfter(
        ...
        callback: LoadCallback<String, IoTDevice>
    ) {
    ...
    private suspend fun requestAndLoadDevices(after: String? = null, loadFinished: (IoTDevicesResponse?) -> Unit) {
        val accountID = "$KEY_ACCOUNT_ID%3D${SharedPrefHelper.getSelectedAccountID()}"
        try {
            val response = if(after != null){
                cloudRepository.getDevices(NETWORK_PAGE_SIZE, accountID, KEY_DESCENDING, after)
            } else {
                cloudRepository.getDevices(NETWORK_PAGE_SIZE, accountID, KEY_DESCENDING)
            }
